...
void malloc_run_state(RunState* s, Config* p) {
    ...
    long long current_alloc_bytes;

    // Helper macro for allocation and checking
    #define DEBUG_CALLOC(ptr, num, size, name_str) \
        current_alloc_bytes = (long long)(num) * (size); \
        printf("Attempting to allocate %s: %lld elements, %lu bytes each, total %lld bytes.\n", name_str, (long long)(num), (unsigned long)(size), current_alloc_bytes); \
        fflush(stdout); \
        ptr = calloc(num, size); \
        if (!ptr) { \
            fprintf(stderr, "malloc failed for %s! Requested %lld bytes. Total allocated so far: %lld bytes.\n", name_str, current_alloc_bytes, total_allocated_bytes); \
            exit(EXIT_FAILURE); \
        } \
        total_allocated_bytes += current_alloc_bytes; \
        printf("Successfully allocated %s. Total allocated so far: %lld bytes.\n", name_str, total_allocated_bytes); \
        fflush(stdout);

    DEBUG_CALLOC(s->x, p->dim, sizeof(float), "s->x");
    DEBUG_CALLOC(s->xb, p->dim, sizeof(float), "s->xb");
    DEBUG_CALLOC(s->xb2, p->dim, sizeof(float), "s->xb2");
    DEBUG_CALLOC(s->hb, p->hidden_dim, sizeof(float), "s->hb");
    DEBUG_CALLOC(s->hb2, p->hidden_dim, sizeof(float), "s->hb2");

    // For QuantizedTensor members, allocate .q and .s separately
    printf("Attempting to allocate s->xq.q: %d elements, %lu bytes each, total %lld bytes.\n", p->dim, (unsigned long)sizeof(int8_t), (long long)p->dim * sizeof(int8_t));
    fflush(stdout);
    s->xq.q = calloc(p->dim, sizeof(int8_t));
    if (!s->xq.q) { fprintf(stderr, "malloc failed for s->xq.q!\n"); exit(EXIT_FAILURE); }
    total_allocated_bytes += (long long)p->dim * sizeof(int8_t);
    printf("Successfully allocated s->xq.q. Total allocated so far: %lld bytes.\n", total_allocated_bytes);
    fflush(stdout);

    size_t xq_s_elements = p->dim / (GS ? GS : 1);
    DEBUG_CALLOC(s->xq.s, xq_s_elements, sizeof(float), "s->xq.s");
    
    printf("Attempting to allocate s->hq.q: %d elements, %lu bytes each, total %lld bytes.\n", p->hidden_dim, (unsigned long)sizeof(int8_t), (long long)p->hidden_dim * sizeof(int8_t));
    fflush(stdout);
    s->hq.q = calloc(p->hidden_dim, sizeof(int8_t));
    if (!s->hq.q) { fprintf(stderr, "malloc failed for s->hq.q!\n"); exit(EXIT_FAILURE); }
    total_allocated_bytes += (long long)p->hidden_dim * sizeof(int8_t);
    printf("Successfully allocated s->hq.q. Total allocated so far: %lld bytes.\n", total_allocated_bytes);
    fflush(stdout);

    size_t hq_s_elements = p->hidden_dim / (GS ? GS : 1);
    DEBUG_CALLOC(s->hq.s, hq_s_elements, sizeof(float), "s->hq.s");

    DEBUG_CALLOC(s->q, p->dim, sizeof(float), "s->q");
    DEBUG_CALLOC(s->k, kv_dim, sizeof(float), "s->k");
    DEBUG_CALLOC(s->v, kv_dim, sizeof(float), "s->v");
    DEBUG_CALLOC(s->att, p->n_heads * p->seq_len, sizeof(float), "s->att");
    DEBUG_CALLOC(s->logits, p->vocab_size, sizeof(float), "s->logits");
    DEBUG_CALLOC(s->key_cache, (size_t)p->n_layers * p->seq_len * kv_dim, sizeof(float), "s->key_cache"); // Cast to size_t for num to avoid overflow if product is large
    DEBUG_CALLOC(s->value_cache, (size_t)p->n_layers * p->seq_len * kv_dim, sizeof(float), "s->value_cache"); // Cast to size_t

    // Original comprehensive check (optional now, as individual checks are more specific)
    // if (!s->x || !s->xb || !s->xb2 || !s->hb || !s->hb2 || !s->xq.q || !s->xq.s || !s->hq.q || !s->hq.s
    //  || !s->q || !s->k || !s->v || !s->att || !s->logits || !s->key_cache
    //  || !s->value_cache) {
    //     fprintf(stderr, "One or more mallocs failed in malloc_run_state! Total allocated before failure: %lld bytes.\n", total_allocated_bytes);
    //     exit(EXIT_FAILURE);
    // }
    printf("malloc_run_state: All allocations successful. Total bytes allocated: %lld\n", total_allocated_bytes);
    fflush(stdout);
}
...
void build_transformer(Transformer *t) {
    t->data = (float*)stories260K_q80_bin; 
    t->file_size = stories260K_q80_bin_len; 
    ...
    // current_ptr += sizeof(int); // This line was in the original, ensure it's correct based on header structure
    GS = group_size_from_data; 
    if (GS == 0) {
        fprintf(stderr, "Error: Group size (GS) read from model data is 0.\n");
        exit(EXIT_FAILURE);
    }
    // Debug prints from Step 1
    printf("build_transformer: GS = %d\n", GS);
    printf("build_transformer: Config.dim = %d\n", t->config.dim);
    printf("build_transformer: Config.hidden_dim = %d\n", t->config.hidden_dim);
    printf("build_transformer: Config.n_layers = %d\n", t->config.n_layers);
    printf("build_transformer: Config.n_heads = %d\n", t->config.n_heads);
    printf("build_transformer: Config.n_kv_heads = %d\n", t->config.n_kv_heads);
    printf("build_transformer: Config.vocab_size = %d\n", t->config.vocab_size);
    printf("build_transformer: Config.seq_len = %d\n", t->config.seq_len);
    fflush(stdout); 
    ...
    printf("Calling malloc_run_state...\n"); 
    fflush(stdout);
